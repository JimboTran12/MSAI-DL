...
class WeatherForecast:
    ...
    def find_the_largest_drop(self) -> torch.Tensor:
        ...
        """
        average_temp = self.data.mean(dim=-1)
        temperature_drop = torch.diff(average_temp)
        return temperature_drop.min()
    ...
    def find_the_most_extreme_day(self) -> torch.Tensor:
        ...
        """
        average_temp = self.data.mean(dim=-1) #(n,)
        diff_data = abs(self.data - average_temp.unsqueeze(1) )#(n, 1)
        idx_max_diff = torch.argmax(diff_data, dim = 1)
        return self.data[range(idx_max_diff.numel()),idx_max_diff]
    ...
    def max_last_k_days(self, k: int) -> torch.Tensor:
        ...
        Returns:
            tensor of size (k,)
        """
        max_temp = self.data.max(1).values[-k:]
        return max_temp
    ...
    def what_day_is_this_from(self, t: torch.FloatTensor) -> torch.LongTensor:
        ...
        """
        norm = torch.norm(self.data - t, dim = -1)
        return torch.argmin(norm)
